...
func _input(event):
	if not auto_shoot and event is InputEventMouseMotion:
		update_body_rotate(get_global_mouse_position())

#Auto shooting
func auto_shooting(nearest_enemy: Vector2):
	if auto_shoot and not is_shot_cd :
		update_body_rotate(nearest_enemy)
		shoot()
		is_shot_cd = true
		shot_timer.start(0.2)
...
func update_body_rotate(direction: Vector2):
	if not rotate_flag:
		return
	# Rotate with mouse
	body_rotate.look_at(direction)
